  ...
  return (
    <div className="p-4 md:p-8 max-w-[1280px] mx-auto space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { icon: Skull, label: "Vafot etganlar", value: DEATH_STATS.total, sub: `${DEATH_STATS.children} bola · ${DEATH_STATS.adults} katta`, color: "#EF4444" },
          { icon: Activity, label: "O'rtacha kechish", value: `${DEATH_STATS.avgDaysToDeath} kun`, sub: "kasallikdan vafotgacha", color: "#F59E0B" },
          { icon: FlaskConical, label: "Lab. tasdiqlangan", value: LAB.confirmed, sub: "N. meningitidis (56,2%)", color: "#10B981" },
          { icon: FileText, label: "Tahlil jarayonda", value: LAB.inProgress, sub: "natija kutilmoqda", color: "#3B82F6" },
        ].map((c) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm">
          <h2 className="text-[16px] font-semibold text-[#111827] mb-1">Laboratoriya natijalari</h2>
          <p className="text-[13px] text-[#6B7280] mb-2">Olingan namunalar bo'yicha</p>
          ...
          <div className="space-y-1.5">
            {labData.map((d) => (
              <div key={d.name} className="flex items-center justify-between text-[13px]">
                <span className="flex items-center gap-2"><span className="w-3 h-3 rounded-sm" style={{ backgroundColor: d.color }} />{d.name}</span>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-[#E5E7EB] shadow-sm overflow-hidden flex flex-col">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-5 border-b border-[#E5E7EB] bg-[#FAFAFA]">
            <div>
              <h2 className="text-[16px] font-semibold text-[#111827]">Vafot etgan beмorlar reestri</h2>
              <p className="text-[13px] text-[#6B7280]">Yakuniy tashxis: yashin tezligidagi meningokokksemiya</p>
            </div>
            <input
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="F.I.Sh yoki tuman..."
              ...
              <thead className="sticky top-0 bg-[#F9FAFB] z-10">
                <tr className="border-b-2 border-[#E5E7EB]">
                  {["#", "F.I.Sh.", "Yosh", "Tuman", "Kasallangan", "Yotqizilgan", "Vafot"].map((h) => (
                    <th key={h} className="px-3 py-2.5 text-[11px] font-bold text-[#6B7280] uppercase tracking-wider whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
          ...
          <div className="px-5 py-3 border-t border-[#E5E7EB] bg-[#FAFAFA] text-[12px] text-[#6B7280]">
            Jami {filtered.length} ta yozuv · {DEATH_STATS.children} nafari (78,9%) bolalar
          </div>
        </div>
      </div>
    </div>
