  ...
  return (
    <div className="p-4 md:p-8 max-w-[1200px] mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-[#E5E7EB] p-6 shadow-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
            {[
              ...
              <div key={item.label} className="flex gap-3">
                ...
                <div>
                  <div className="text-[11px] text-[#9CA3AF] uppercase tracking-wide">{t(item.label)}</div>
          ...
          <div className="border-t border-[#E5E7EB] pt-4">
            <h3 className="text-[13px] font-semibold text-[#111827] uppercase tracking-wide mb-3">{t("Tarkibi (1 doza)")}</h3>
      ...
      <div className="bg-white rounded-2xl border border-[#E5E7EB] p-6 shadow-sm">
        <h2 className="text-[16px] font-semibold text-[#111827] mb-5">{t("O'choqda profilaktika bosqichlari")}</h2>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {SCHEDULE.map((s, i) => (
            <div key={s.step} className="relative p-4 rounded-xl border border-[#E5E7EB] bg-[#FAFAFA]">
              ...
              <h3 className="text-[14px] font-semibold text-[#111827]">{t(s.title)}</h3>
              <p className="text-[12px] text-[#6B7280] mt-1">{t(s.desc)}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-[15px] font-semibold text-[#111827]">{t("Ko'rsatmalar")}</h2>
          </div>
          <ul className="space-y-2.5">
            {INDICATIONS.map((item) => (
              <li key={item} className="flex gap-2 text-[13px] text-[#4B5563]">
                <span className="text-[#10B981] mt-0.5">•</span>{t(item)}
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-[15px] font-semibold text-[#111827]">{t("Qarshi ko'rsatmalar")}</h2>
          </div>
          <ul className="space-y-2.5">
            {CONTRAINDICATIONS.map((item) => (
              <li key={item} className="flex gap-2 text-[13px] text-[#4B5563]">
                <span className="text-[#EF4444] mt-0.5">•</span>{t(item)}
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-[15px] font-semibold text-[#111827]">{t("Nojo'ya ta'sirlar")}</h2>
          </div>
          <div className="mb-3">
            <p className="text-[11px] text-[#9CA3AF] uppercase tracking-wide mb-1">{t("Mahalliy")}</p>
            <p className="text-[13px] text-[#4B5563]">{SIDE_EFFECTS.local.map((x) => t(x)).join(", ")}</p>
          </div>
          <div className="mb-3">
            <p className="text-[11px] text-[#9CA3AF] uppercase tracking-wide mb-1">{t("Tizimli")}</p>
            <p className="text-[13px] text-[#4B5563]">{SIDE_EFFECTS.systemic.map((x) => t(x)).join(", ")}</p>
          </div>
          <p className="text-[12px] text-[#9CA3AF] italic">{t(SIDE_EFFECTS.note)}</p>
        </div>
      </div>
    </div>
  );
